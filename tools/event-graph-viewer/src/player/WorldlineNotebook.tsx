...
export function WorldlineNotebook({ save, currentLoop, knownDiff }: { save: PlayerSave; currentLoop: number; knownDiff: (save: PlayerSave, a: number, b: number) => KnownDiff }) {
  ...
  return <div className="worldlines"><p>只比較你拿到的紀錄。空白的地方，鎮上可能已經發生了事，只是你還不知道。</p>
    ...
    <div className="worldline-pair"><section><small>第 {left} 次</small><strong>{diff.left}</strong><p>{visibleRecords(save, left).some(r => r.id === 'reporter-message') ? '21:20：予安說庭安失去聯絡' : '21:14：尚未查到消息'}</p></section><section><small>第 {right} 次</small><strong>{diff.right}</strong><p>{visibleRecords(save, right).some(r => r.id === 'reporter-message') ? '21:20：予安說庭安失去聯絡' : '21:14：尚未查到消息'}</p></section></div>
